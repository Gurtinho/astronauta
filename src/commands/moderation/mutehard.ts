...
export default new Command({
    ...
    async run({ interaction, options }) {
        ...
        try {
            ...
            const botName = interaction.guild.roles.cache.find((role) => role.name == client.user?.username);
            if (!mutedRole) {
            ...
            if (mutedRole) {
                interaction.guild.channels.cache.forEach(channel => {
                    if (channel.type == ChannelType.GuildText) {
                        channel.permissionOverwrites.create(mutedRole!, { SendMessages: false });
                    }
                    if (channel.type == ChannelType.GuildVoice) {
                        channel.permissionOverwrites.create(mutedRole!, { Connect: false });
                    }
                });
            }
