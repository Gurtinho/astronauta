...
import { Welcome } from '../../database/entities/Welcome';
import { dataConnection } from '../../database/data-source';
const welcomeRepository = dataConnection.getRepository(Welcome);
...
export default new Command({
    ...
    async run({ interaction, options }) {
        ...
        try { 
            ...
            const welcomeData = await welcomeRepository.findOneBy({
                guild: interaction.guild.id
            });
            if (!welcomeData) {
                const welcomeCreated = await welcomeRepository.save({
                    guild: interaction.guild.id,
            ...
            return interaction.reply({
                content: 'Ocorreu um erro ao tentar setar o canal de boas vindas.',
                ephemeral: true
            });
        }
    }
